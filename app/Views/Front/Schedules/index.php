...
const getServices = async () => {
    ...
    // Obtém a referência do elemento 'select' com o ID 'service_id'
    ...
    elementService.addEventListener('change', (event) => {
        // Evento disparado quando o usuário seleciona ou desmarca uma opção no dropdown

        // Obtém o valor da opção selecionada ou define como nulo caso não haja seleção
        serviceId = elementService.value ?? null;

        // Obtém o texto da opção selecionada no dropdown, se uma opção estiver selecionada
        // Caso contrário, define o texto como nulo
        let serviceName = serviceId !== '' ? elementService.options[event.target.selectedIndex].text :
            null;

        // Registra uma mensagem no console para indicar se um serviço foi escolhido ou não
        console.log('Serviço foi escolhido? ', serviceId !== '');

        // Atualiza um elemento do DOM com o nome do serviço escolhido
        chosenServiceText.innerText = serviceName;

        // Mostra ou esconde outro elemento (provavelmente um contêiner) com base na seleção do serviço
        // Se o serviço foi escolhido (serviceId !== ''), remove a classe 'd-none' para mostrar o elemento
        // Caso contrário, adiciona a classe 'd-none' para esconder o elemento
